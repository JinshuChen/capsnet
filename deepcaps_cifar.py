...
#     self.w = nn.Parameter(torch.randn(ch_j*n_j, 1, n_i, 3, 3)).cuda()
  ...
  def forward(self, inputs):
    ...
    x = inputs.view(self.batch, self.ch_i * self.n_i, self.h_i, self.w_i)
    x = x.unsqueeze(1)  # (batch, 1, ch_i * n_i, h_i, w_i)
    x = self.conv1(x)
    width = x.shape[-1]
    
    x = x.permute(0,2,1,3,4)
    x = x.view(self.batch, self.ch_i, self.ch_j, self.n_j, width, width)  # 为什么先permute再view？
...
class Model(nn.Module):
    def __init__(self):
        ...
        self.digCaps = CapsuleLayer(num_capsules=10, num_routes=64, in_channels=8, out_channels=16, routing_iters=3)
    ...
    def forward(self, x, target=None):
        ipdb.set_trace()
        x = self.conv2d(x)   #[B,128,32,32]
        x = self.batchNorm(x)
        x = self.toCaps(x) #[B,128,1,32,32]
        
        x = self.conv2dCaps1_nj_4_strd_2(x)  #[B,32,4,16,16]
        x_skip = self.conv2dCaps1_nj_4_strd_1_1(x)  #[B,32,4,16,16]
        x = self.conv2dCaps1_nj_4_strd_1_2(x)  #[B,32,4,16,16]
        x = self.conv2dCaps1_nj_4_strd_1_3(x)  #[B,32,4,16,16]
        x = x + x_skip  #[B,32,4,16,16]
        
        x = self.conv2dCaps2_nj_8_strd_2(x) #[B,32,8,8,8]
    ...
    def loss(self, x, recnstrcted, data, labels, lamda=0.5, m_plus=0.9, m_minus=0.1):
        ...
        loss = self.margin_loss(x, labels, lamda, m_plus, m_minus)
        return loss.mean()
...
# ## Fashion MNIST
# train_loader = torch.utils.data.DataLoader(datasets.FashionMNIST(root='/home/chenjinshu/capsnet/data/FashionMNIST/FashionMNIST/',train=True,download=True,transform=trans(rotation_range=0.1, translation_range=0.1, zoom_range=(0.1, 0.2))),batch_size=batch_size,shuffle=True)
# test_loader = torch.utils.data.DataLoader(datasets.FashionMNIST(root='/home/chenjinshu/capsnet/data/FashionMNIST/FashionMNIST/',train=False,download=True,transform=transforms.ToTensor()),batch_size=batch_size,shuffle=True)

## CIFAR10
trans1 = transforms.Compose([transforms.RandomCrop(5),transforms.RandomHorizontalFlip(p=0.5),transforms.RandomVerticalFlip(p=0.5),transforms.ToTensor(),transforms.Normalize((0.49139968, 0.48215827, 0.44653124), (0.24703233, 0.24348505, 0.26158768))])
trans2 = transforms.Compose([transforms.ToTensor(),transforms.Normalize((0.49139968, 0.48215827, 0.44653124), (0.24703233, 0.24348505, 0.26158768))])
train_loader = torch.utils.data.DataLoader(datasets.CIFAR10(root='/home/chenjinshu/capsnet/data/CIFAR',train=True,download=False,transform=trans2),batch_size=batch_size,shuffle=True)
test_loader = torch.utils.data.DataLoader(datasets.CIFAR10(root='/home/chenjinshu/capsnet/data/CIFAR',train=False,download=False,transform=trans2),batch_size=batch_size,shuffle=True)
...
def test(model, test_loader, loss, batch_size, Te_loss,Te_acc,lamda=0.5, m_plus=0.9, m_minus=0.1):
  test_loss = 0.0
  correct = 0.0
...
#         print(indices_cpu)
  ...
  Te_loss.append(float(test_loss.item()/len(test_loader.dataset)))
  Te_acc.append(float(correct/len(test_loader.dataset) * 100))
...
def train(train_loader, model, num_epochs, lr=0.001, batch_size=64, lamda=0.5, m_plus=0.9,  m_minus=0.1):
    ...
    Tr_loss, Itr, Tr_acc, Te_loss, Te_acc= [],[],[],[],[]
    ...
    for epoch in range(num_epochs):
      for batch_idx, (data, label_) in enumerate(train_loader):
        ...
        if batch_idx%100 == 0:
          ...
          loss_val = model.loss(outputs, recnstrcted, data, labels, lamda, m_plus, m_minus)
          print("epoch: ", epoch, "batch_idx: ", batch_idx, "loss: ", loss_val/batch_size, "accuracy: ", accuracy(indices, label_.cpu())/indices.shape[0])
          Tr_loss.append(float(loss_val.item() / batch_size))
          Tr_acc.append(float(accuracy(indices, label_.cpu())/indices.shape[0]) * 100)
          Itr.append(epoch+batch_idx/1000)
      test(model, test_loader, loss, batch_size, Te_loss,Te_acc,lamda, m_plus, m_minus)
      lr_scheduler.step()

    fig, (ax1, ax2) = plt.subplots(2, 1)
    fig.subplots_adjust(hspace=0.5)
    ax1.plot(Itr,Tr_loss,list(range(num_epochs)),Te_loss)
    ax1.set_xlabel('Itrs')
    ax1.set_ylabel('loss')
    ax1.grid(True)
    ax2.plot(Itr,Tr_acc,list(range(num_epochs)),Te_acc)
    ax2.set_xlabel('Itrs')
    ax2.set_ylabel('acc')
    ax2.grid(True)
    fig.tight_layout()
...
# soft-training
# train(train_loader, model, num_epochs=4, lr=0.001, batch_size=256, lamda=0.5, m_plus=0.9,  m_minus=0.1)
